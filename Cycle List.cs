...
{
    public partial class Cycle_List : Form
    {
        private string currentUser;

        public Cycle_List(string username)
        {
            InitializeComponent();
        ...
        private void LoadVehicleData()
        {
            try
            {
                ...
                {
                    string query = "SELECT reg, model, color, fuel, transmission, RENTED_BY, picture FROM VEHICLE WHERE type = 'Cycle'";
            ...
            if (e.RowIndex >= 0 && dataGridView1.Columns[e.ColumnIndex].Name == "ViewPhoto")
            {
                try
                {
                    ...
                    if (cellValue != DBNull.Value)
                        ...
                        MessageBox.Show("No image stored for this cycle.");
                }
                catch (Exception ex)
                {
        ...
        private void ShowImagePopup(byte[] imgBytes)
        {
            try
            {
                using (MemoryStream ms = new MemoryStream(imgBytes))
                {
                    ...
                    previewForm.Text = "Cycle Preview";
        ...
        // Unused
        private void Cycle_List_Load(object sender, EventArgs e) { }
        private void Cycle_List_Load_1(object sender, EventArgs e) { }
